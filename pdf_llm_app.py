...
# A user persona that reads articles and makes purchasing decisions based on the content of the article and being very critical and hard to convince.
...
write_task = Task(
  ...
  This article should be easy to understand, engaging, and positive. Use AIDA technique of writing to convince readers. It is more like a sales pages and should tackle potential buyers questions.""",
  expected_output=f'A SEO optimized article with: An engaging title,  4 paragraph article on {topic} with sub-titles, 3 questions for the FAQ with answers, 3 hashtags and 10 comma separated keywords. All fromated as markdown. with one image prompt suggestion to create an image to illustrate this article using llm image generators.',
  tools=[search_tool], # if process=Process.sequential can put tool here as well but if it is hierarchical put it only at the agent level not task level for fluidity
  agent=writer,
...
critical_buyer_persona_task = Task(
  ...
  expected_output=f'Write a report in your final purchasing decison after having red the article and your detailed motivation. Make sure that it is well structured and easy to understand for people who are not expert like you in online purchase.',
  tools=[search_tool], # if process=Process.sequential can put tool here as well but if it is hierarchical put it only at the agent level not task level for fluidity
  agent=critical_buyer_persona,
  async_execution=False,
