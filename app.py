...
class AgentTeam:
  ...
  def DatabaseRetriever(self):
    self.database_retriever = Agent(
      ...
      goal=f"You will find information in the database using the database_retriever_tool about the '{self.topic}'. After getting the answer from the database, you will analyze it and analyze the topic and produce a report for the user to have insights about the '{topic}' and be ready for its next meeting with some information about the '{self.topic}'",
      verbose=True,
      memory=True,
...
class TaskGroups:
  ...
  def OnlineSearchTask(self):
    self.online_search = Task(
      ...
      agent=AgentTeam(topic=self.topic).OnlineSearcher(),
  ...
  def JudgeDataTask(self):
    self.judge_data = Task(
      ...
      agent=AgentTeam(topic=self.topic).CriticalResults(),
      async_execution=False,
  ...
  def ProduceReportTask(self):
    self.produce_report = Task(
      ...
      agent=AgentTeam(topic=self.topic).ReportCreator(),
      async_execution=False,
